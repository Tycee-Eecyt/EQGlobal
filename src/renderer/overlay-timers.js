...
function renderTimers() {
  ...
  const rowsHtml = timers
    .map((timer) => {
      ...
      return `
        <div class="timer-row ${soon ? 'soon' : ''}" data-exp="${expiresAt}" data-dur="${totalMs}" data-id="${escapeHtml(timer.id || '')}">
          <span class="time">${formatHMS(remainingSeconds)}</span>
...
function startTimerAnimation() {
  ...
  const step = () => {
    const now = Date.now();
    const rows = timersContainer ? timersContainer.querySelectorAll('.timer-row') : [];
